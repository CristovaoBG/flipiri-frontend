<template>
    ...
    <div v-show="showNewActivity || showEditActivity ">
      ...
      <form @submit.prevent="handleSubmit">
        <label for="name">Nome da hospedagem:</label><input type="text" id="name" v-model="formData.name" required>
        <br>
        <label for="vacancies">Vagas:</label> <input type="number" id="vacancies" v-model="formData.vacancies" required>
        <br>
        <label for="price">Preço do pernoite por pessoa:</label> <input type="number" id="price" v-model="formData.price" step="0.01" required class="no-spin">
        <br><br>
...
const handleSubmit = () => {
  ...
  postData('add_hosting/', data, (response) => {
